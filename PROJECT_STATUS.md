...
- **Đã cài bộ hook/subagent/skill trong `.claude/`** (chưa commit): hook `no-secrets.js`/`block-dangerous.js`/`test-gate.js`/`format-on-edit.js` (đăng ký trong `.claude/settings.json`), subagent `code-reviewer` (Haiku), `security-reviewer` (model mặc định, dùng cho Auth Service), `test-runner` (Haiku), skill `/deploy`. Lưu ý quan trọng: **các hook này chỉ chạy khi git operation đi qua Claude Code (Bash tool/skill `/deploy`) — không chạy khi push bằng Fork/tay**, vì đây là cơ chế riêng của Claude Code, không phải git hook thật (`.git/hooks/`). Muốn áp dụng cả khi dùng Fork thì phải làm thêm git hook thật riêng (`.githooks/` + `core.hooksPath`) — chưa làm.
- **Cloudflare Tunnel (`cloudflared`) cần outbound cổng 7844 (TCP+UDP) tới Cloudflare edge, không có cách fallback sang cổng 443** (đã verify qua doc chính thức Cloudflare) — nếu mạng chặn cổng này thì tunnel treo `i/o timeout`, không phải lỗi Coolify/VM. Domain public lúc đó NXDOMAIN hoặc không vào được, nhưng vào thẳng qua IP nội bộ VMware NAT (`192.168.48.162:8000`) vẫn bình thường vì không cần tunnel.
